...
#	include "playable_z.hpp" // active floor/layer/revision context
...
namespace
{
	...
	int tileAt(int layer, int x, int y)
	{
		...
		return map.tileAt(x, y, layer, activeRuntimePlayableFloor());
	}
	...
	{
		Entity* entity = uidToEntity((Sint32)uid);
		if ( entity && entity->playableFloor != activeRuntimePlayableFloor() )
		{
			return nullptr;
		}
		return entity;
...
bool SAMWorld::setTile(int x, int y, int layer, int tileId)
{
	...
	// Upstream SAMT has no registry negotiation, MapInstance, or playable-floor
	// field. Refuse every multiplayer terrain mutation instead of applying a
	// server-only change or sending an ambiguous packet. Adding those fields is
	// the protocol redesign this integration intentionally stops before.
	if ( multiplayer != SINGLE )
	{
		SAM_WARN(MOD, "sam_set_tile refused in multiplayer: the S.A.M 2.1 tile packet has no spatial context.");
		return false;
	}
	const PlayableFloorId floor = activeRuntimePlayableFloor();
	if ( !map.setTileAt(x, y, layer, tileId, floor) ) { return false; }
...
bool SAMWorld::connected(int x1, int y1, int x2, int y2, bool flying)
{
	...
	if ( activeRuntimePlayableFloor() != DEFAULT_PLAYABLE_FLOOR )
	{
		SAM_WARN(MOD, "sam_tiles_connected is unavailable off playable floor zero until path-map queries accept a floor.");
		return false;
	}
	...
	const std::string& kind)
{
	...
	for ( node_t* node = map.entities->first; node != nullptr; node = node->next )
	{
		...
		if ( !e ) { continue; }
		if ( e->playableFloor != activeRuntimePlayableFloor() ) { continue; }
