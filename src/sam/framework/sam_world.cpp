...
bool SAMWorld::setTile(int x, int y, int layer, int tileId)
{
	...
	// SAMT has no registry negotiation, MapInstance, or playable-floor field.
	// Refuse multiplayer terrain mutation rather than sending an ambiguous update
	// that would modify only the server's active floor.
	if ( multiplayer != SINGLE )
	{
		SAM_WARN(MOD, "sam_set_tile refused in multiplayer: the S.A.M 2.8 tile packet has no spatial context.");
		return false;
	}
