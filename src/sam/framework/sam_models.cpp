...
int SAMModels::appendModels(const std::vector<Request>& requests)
{
	...
	// Keep the numeric model table identical on headless and graphical peers. The
	// dedicated server needs ids for authoritative entities but must not touch GPU
	// buffers; graphical builds create geometry/VBOs at the normal post-reload seam.
	if ( !headless )
	{
