...
const PlayerSlotLookup<Uint32, MAXPLAYERS>&
getLootBagVariationByPlayerLookup(const bool colorblind)
{
	static const Uint32 normalPrimary[] = {
		0, 1, 2, 3, 4
	};
	static const Uint32 normalCycle[] = {
		2, 3, 4
	};
	static const Uint32 colorblindPrimary[] = {
		2, 3, 1, 4, 5, 6, 7, 8
	};
	static const Uint32 colorblindCycle[] = {
		5, 6, 7, 8
	};

	static const PlayerSlotLookup<Uint32, MAXPLAYERS>
		normalVariationByPlayer =
			buildPlayerSlotLookup<Uint32, MAXPLAYERS>(
				normalPrimary,
				normalCycle,
				kLootBagFallbackVariation
			);
	static const PlayerSlotLookup<Uint32, MAXPLAYERS>
...
const PlayerSlotLookup<int, MAXPLAYERS>&
getLootBagLightPaletteByPlayerLookup()
{
	static const int lightPrimary[] = {
		0, 1, 2, 3, 4
	};
	static const int lightCycle[] = {
		2, 3, 4
	};
...
{
	const int fallbackVariation =
		std::max(
			0,
			std::min(
				kLootBagFallbackVariation,
				items[TOOL_PLAYER_LOOT_BAG].variations - 1
			)
		);
	...
	const int variation =
		static_cast<int>(
			getLootBagVariationByPlayerLookup(
				colorblind
			)[playerOwner]
		);
	...
	if ( variation >= 0
		&& variation
			< items[TOOL_PLAYER_LOOT_BAG].variations )
	{
		return variation;
	}
...
)
{
	...
	const auto& variationByPlayer =
		getLootBagVariationByPlayerLookup(colorblind);
...
)
{
	...
	return getLootBagLightPaletteByPlayerLookup()[
		playerOwner
	];
}
...
{
	return getLootBagLightPaletteForPlayer(
		getLootBagPlayerForVariation(
			variation,
			colorblind
		)
	);
